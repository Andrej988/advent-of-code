...
const findBadge = (line) => {
  group.push(line);

  if (group.length == 3) {
    let badgeCharacter = findBadgeCharacter();
    badgeCharacter.forEach((x) => {
      if (x.match("[a-zA-Z]")) {
...
const findBadgeCharacter = () => {
  const set1 = new Set(group.pop());
  const set2 = new Set(group.pop());
  const set3 = new Set(group.pop());

  let intersect = new Set([...set1].filter((x) => set2.has(x) && set3.has(x)));
  return intersect;
};
