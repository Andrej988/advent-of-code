...
use super::{
    coordinates::{new_coordinates, perform_move, Coordinates},
    direction::Direction,
};
...
pub fn solve_challenge(map: &mut HashMap<Coordinates, char>) {
    ...
    let precipe_x = map
        ...
        .unwrap()
        + 1;
    println!("Prexipe X: {}", precipe_x);

    let mut num_of_units: u16 = 0;
    ...
    println!("Total number of units: {}", num_of_units);
}
...
fn sandfall(
    ...
    loop {
        ...
        {
            sand_coords = perform_move(&sand_coords, &Direction::DownAndLeft);
        } else if is_coordinate_reachable(
            &perform_move(&sand_coords, &Direction::DownAndRight),
            map,
        ) {
...
fn is_coordinate_reachable(coords: &Coordinates, map: &HashMap<Coordinates, char>) -> bool {
